...
class Quaternion {
    ...
    val roll: Double
        get() {
            normalize()
            val pole = gimbalPole
            return if (pole == 0) Math.atan2(2.0 * (w * z + y * x), 1.0 - 2.0 * (x * x + z * z)) else pole.toDouble() * 2.0 * Math.atan2(y, w)
        }
    ...
    val pitch: Double
        get() {
            normalize()
            val pole = gimbalPole
            return if (pole == 0) Math.asin(MathUtil.clamp(2.0 * (w * x - z * y), -1.0, 1.0)) else pole.toDouble() * MathUtil.PI * 0.5
        }
    ...
    val yaw: Double
        get() {
            normalize()
            return if (gimbalPole == 0) Math.atan2(2.0 * (y * w + x * z), 1.0 - 2.0 * (y * y + x * x)) else 0.0
        }
     ...
     * @return A reference to this [Quaternion] to facilitate chaining.
     */
    fun fromAngleAxis(axis: Axis, angle: Double): Quaternion {
    ...
    fun fromAngleAxis(axis: Vector3, angle: Double): Quaternion {
        ...
        val halfAngleSin = Math.sin(halfAngle)
    ...
    fun fromAngleAxis(x: Double, y: Double, z: Double, angle: Double): Quaternion {
        ...
        val l_ang = if (radians < 0) MathUtil.TWO_PI - -radians % MathUtil.TWO_PI else radians % MathUtil.TWO_PI
        val l_sin = Math.sin(l_ang * 0.5)
        val l_cos = Math.cos(l_ang * 0.5)
     ...
     * @return A reference to this [Quaternion] to facilitate chaining.
     */
    fun fromAxes(xAxis: Vector3, yAxis: Vector3, zAxis: Vector3): Quaternion {
    ...
    fun fromEuler(yawIn: Double, pitchIn: Double, rollIn: Double): Quaternion {
        ...
        val num9 = roll * 0.5
        val num6 = Math.sin(num9)
        val num5 = Math.cos(num9)
        val num8 = pitch * 0.5
        val num4 = Math.sin(num8)
        val num3 = Math.cos(num8)
        val num7 = yaw * 0.5
        val num2 = Math.sin(num7)
        val num = Math.cos(num7)
        val f1 = num * num4
        val f2 = num2 * num3
        val f3 = num * num3
        val f4 = num2 * num4
    ...
    fun normalize(): Double {
        val len = length2()
        if (len != 0.0 && Math.abs(len - 1.0) > NORMALIZATION_TOLERANCE) {
            val factor = 1.0 / Math.sqrt(len)
            multiply(factor)
        }
        return len
    }
     ...
     */
    fun exp(): Quaternion {
        val angle = Math.sqrt(x * x + y * y + z * z)
    ...
    fun slerp(end: Quaternion, t: Double): Quaternion {
        ...
        if (1 - absDot > 0.1) {
            ...
            val invSinTheta = 1 / Math.sin(angle)
    ...
    fun slerp(q1: Quaternion, q2: Quaternion, t: Double): Quaternion {
        ...
        if (1 - result > 0.1) {
            ...
            val invSinTheta = 1 / Math.sin(theta)
    ...
    fun lookAt(lookAt: Vector3, upDirection: Vector3): Quaternion {
        ...
        val dotError = Math.abs(Math.abs(dotProduct) - lookAt.length() * upDirection.length())
        if (dotError <= 1e-6) {
    ...
    companion object {
        //Tolerances
        val F_EPSILON = .001
        val NORMALIZATION_TOLERANCE = 0.00001
         ...
         */
        fun lerp(rkP: Quaternion, rkQ: Quaternion, t: Double, shortestPath: Boolean): Quaternion {
            sTmp1.setAll(rkP)
            sTmp2.setAll(rkQ)
